...
function syncVideoViewport(instance) {
  ...
  const totalWallWidth = Math.max(1, Number(wall?.totalWallWidth) || window.innerWidth)
  const renderHeight = Math.max(1, Number(wall?.renderHeightPx) || Number(selfClient?.stackHeight) || window.innerHeight)
  const xStart = Number(selfClient?.xStart) || 0
  const yStart = Number(selfClient?.yStart) || 0

  videoEl.style.width = `${totalWallWidth}px`
  videoEl.style.height = `${renderHeight}px`
...
Template.TelevisionPage.helpers({
  ...
  xStart() {
    const instance = Template.instance()
    return WallClients.findOne({ _id: instance.clientId, wallId: DEFAULT_WALL_ID })?.xStart ?? 0
  },
  yStart() {
    const instance = Template.instance()
    return WallClients.findOne({ _id: instance.clientId, wallId: DEFAULT_WALL_ID })?.yStart ?? 0
  },
  currentSource() {
