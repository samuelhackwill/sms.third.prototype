...
- Production (Mac mini) and test machine use separate local Mongo instances.
- Production and test machine keep separate local raw log directories.
- Checkpoints are local to each machine/environment and are not shared.
...
- `imports/server/rawLog.js`:
  ...
  - Writes to hot files:
    - `data/raw/hot-osx_messages_app.ndjson`
    - `data/raw/hot-sim_router.ndjson`
  - Selects target file by `source`
...
- Exactly two hot files are used (one per source), both append-only during runtime.
...
- ask confirmation that you found the legacy code file
- check legacy code file and make sure it's updated to our news specs
...
**Goal:** Operator-triggered batch import from two hot raw logs into `Messages` idempotently.

**Implement**

- Meteor method: `admin.importRaw({ sources? })`
- Default behavior imports both hot files (`osx_messages_app` and `sim_router`).
- temp pause of the scrapers so no accidental write on file
- Safe drain step per source:
  - atomically rename hot file to a batch file (`imports/<timestamp>_<source>.ndjson`)
  - recreate empty hot file immediately
- Parses NDJSON line-by-line
- Produces canonical record
- Upserts into Mongo
- Returns counts/errors (overall and per source)
- resume scrapers

**Acceptance**

- Re-running import doesn’t duplicate messages.
- Import reports how many lines were processed/inserted/updated, including per-source counts.
...
- status of hot log file (if not empty, show warning that data is ready to be consumed)
- Big button calls `admin.importRaw`
- Shows:
  - last run timestamp
  - per-source counts and errors
  - drained batch filenames

**Acceptance**

- One click drains both hot files, imports them, and reports to console.
